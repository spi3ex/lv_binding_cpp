...
void drm_flush(lv_display_t *disp_drv, const lv_area_t *area, lv_color_t *color_p)
{
	struct drm_buffer *fbuf = drm_dev.cur_bufs[1];
	lv_grid_align_t w = (area->x2 - area->x1 + 1);
	lv_grid_align_t h = (area->y2 - area->y1 + 1);
	int i, y;
...
void drm_get_sizes(lv_grid_align_t *width, lv_grid_align_t *height, uint32_t *dpi)
{
	if (width)
		*width = drm_dev.width;
